#pragma once
#if !defined(MEOW_INVERTIBILITY_AESNI_H)
...
// 将MEOW_MIX_REG、MEOW_SHUFFLE均逆向如下
...
//
void meow_inv(const char* message, meow_umm Len, void* hashvalue)
{
	...
	//meow_u8* rcx = (meow_u8*)Seed128Init;   // MeowHash函数的初始种子，也就是密钥吧
	meow_u8* rcx = (meow_u8*)hashvalue;
	
	//将hashvalue赋值回xmm
	movdqu(xmm0, rcx + 0x00);
	// PrintKey(xmm0); printf("\n");
...
MixDown:
	...
	meow_u8* Last = (meow_u8*)hashvalue + (Len & ~0xf);
	...
	if (Len8)
	{
		...
		meow_u8* LastOk = (meow_u8*)((((meow_umm)(((meow_u8*)hashvalue) + Len - 1)) | (MEOW_PAGESIZE - 1)) - 16);
	...
	PrintKey(xmm0);PrintKey(xmm1);
	printf("\n");
	PrintKey(xmm2);PrintKey(xmm3);
	printf("\n");
	PrintKey(xmm4);PrintKey(xmm5);
	printf("\n");
	PrintKey(xmm6);PrintKey(xmm7);
}
...
#define MEOW_INVERTIBILITY_AESNI_H
#endif